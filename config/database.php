...
class Database
{
    ...
    private static function value(
        ...
        if (
            ...
            && ($trim ? trim($environmentValue) !== '' : $environmentValue !== '')
        ) {
            return $trim ? trim($environmentValue) : $environmentValue;
        }

        if (array_key_exists($localKey, $local)) {
            $localValue = (string) $local[$localKey];
            return $trim ? trim($localValue) : $localValue;
        }
