...
if (empty($_SESSION['user_email'])) {
  header("Location: ../login.php?to=admin/add-initiative.php");
  exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $agreementName = $agreements[$agreementCode]['agreement_name'] ?? '';
...
$data['beneficiaries'] = (string)($male + $female);

$data['supports_qs'] = $qsSupport;
$data['supports_greenmetric'] = $greenMetricSupport;
$data['supports_the'] = '';

$data['sdg_primary'] = $sdgMain;
$data['sdg_secondary'] = $sdgSecondary;
...
$data['notes_entity'] = $notesEntity;
$data['notes_vppd'] = $notesVppd;

/* لأنها مبادرات قديمة ومنتهية، نخليها معتمدة مباشرة */
$data['status'] = 'معتمد';
...
.init-actions{
  ...
  border-top:1px solid rgba(230,235,242,.95) !important;
}
...
/* العربي: السابق/التالي يمين، الحفظ/إلغاء يسار */
[dir="rtl"] .init-actions-left{
  order:1 !important;
}

[dir="rtl"] .init-actions-right{
  order:2 !important;
}

/* الإنجليزي */
[dir="ltr"] .init-actions-left{
  order:1 !important;
}

[dir="ltr"] .init-actions-right{
  order:2 !important;
}
      ...
      <div class="init-agreement-box <?= ($selectedAgreementCode && isset($agreements[$selectedAgreementCode])) ? '' : 'init-hidden' ?>" id="agreementInfoBox">
        <div class="init-agreement-grid">
             <div class="init-agreement-item">
               ...
               <strong id="ag_name"><?= h($agreements[$selectedAgreementCode]['agreement_name'] ?? '—') ?></strong>
             </div>

             <div class="init-agreement-item">
               <span><?= t('agreement_type') ?></span>
                <strong id="ag_type"><?= h($agreements[$selectedAgreementCode]['agreement_type'] ?? '—') ?></strong>
             </div>

            <div class="init-agreement-item">
               <span><?= t('partner_entity') ?></span>
               <strong id="ag_partner"><?= h($agreements[$selectedAgreementCode]['partner_entity'] ?? '—') ?></strong>
             </div>

            <div class="init-agreement-item">
                <span><?= t('country') ?></span>
                <strong id="ag_country"><?= h($agreements[$selectedAgreementCode]['country'] ?? '—') ?></strong>
             </div>

             <div class="init-agreement-item">
                 <span><?= t('responsible_entity') ?></span>
                 <strong id="ag_owner"><?= h($agreements[$selectedAgreementCode]['owner_entity'] ?? '—') ?></strong>
               </div>

              <div class="init-agreement-item">
                 <span><?= t('agreement_status') ?></span>
                 <strong id="ag_status"><?= h($agreements[$selectedAgreementCode]['status'] ?? '—') ?></strong>
               </div>
          </div>
      </div>
        ...
        <div class="col-md-6">
          <label class="init-label"><?= t('brief_summary') ?></label>
          <input class="form-control init-input" name="mini_summary" value="<?= h($_POST['mini_summary'] ?? '') ?>" placeholder="<?= t('optional') ?>">
        </div>
      </div>
    </div>
...
<!-- 🔥 الوصف + suggestions -->
  ...
  <div class="init-help">
    ...
    <div class="init-tab-pane" id="tab-ranking">
      ...
      <div class="row g-4">
        <div class="col-md-6">
          <label class="init-label"><?= t('qs_support') ?></label>
          <?php $qs = $_POST['هل تدعم QS؟'] ?? ''; ?>
          <select class="form-select init-input" name="هل تدعم QS؟">
            <option value=""><?= t('select') ?></option>
            <option value="نعم" <?= $qs === 'نعم' ? 'selected' : '' ?>><?= t('yes') ?></option>
            <option value="لا" <?= $qs === 'لا' ? 'selected' : '' ?>><?= t('no') ?></option>
          </select>
        </div>

        <div class="col-md-6">
          <label class="init-label"><?= t('greenmetric_support') ?></label>
          <?php $gm = $_POST['هل تدعم GreenMetric؟'] ?? ''; ?>
          <select class="form-select init-input" name="هل تدعم GreenMetric؟">
            <option value=""><?= t('select') ?></option>
            <option value="نعم" <?= $gm === 'نعم' ? 'selected' : '' ?>><?= t('yes') ?></option>
            <option value="لا" <?= $gm === 'لا' ? 'selected' : '' ?>><?= t('no') ?></option>
          </select>
        </div>
...
🤖 اقتراح ذكي للأهداف
    ...
    <div class="init-tab-pane" id="tab-docs">
      ...
      <div class="row g-4">
        <div class="col-12">
          <?php $pub = $_POST['هل نُشرت على موقع الجامعة؟'] ?? ''; ?>
          <label class="init-label"><?= t('published_question') ?></label>
          <div class="init-choice-grid">
            <label class="init-radio-card">
              <input type="radio" name="هل نُشرت على موقع الجامعة؟" value="نعم" <?= $pub === 'نعم' ? 'checked' : '' ?>>
              <?= t('yes') ?>
            </label>
            <label class="init-radio-card">
              <input type="radio" name="هل نُشرت على موقع الجامعة؟" value="لا" <?= $pub === 'لا' ? 'checked' : '' ?>>
              <?= t('no') ?>
            </label>
          </div>
        </div>
        ...
        <div class="col-md-6">
          <label class="init-label"><?= t('vppd_notes') ?></label>
          <textarea class="form-control init-input" name="m_notes_vppd" rows="5" placeholder="<?= t('optional') ?>"><?= h($_POST['m_notes_vppd'] ?? '') ?></textarea>
        </div>
      </div>
    </div>
...
(function(){
  ...
  function updateAgreementInfo(){
   ...
   setText('ag_name', data['agreement_name'] || '—');
setText('ag_type', data['agreement_type'] || '—');
setText('ag_partner', data['partner_entity'] || '—');
setText('ag_country', data['country'] || '—');
setText('ag_owner', data['owner_entity'] || '—');
setText('ag_status', data['status'] || '—');
  }
