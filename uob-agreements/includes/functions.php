...
/**
 * Public PostgreSQL Agreement catalogue.
 *
 * This function deliberately has a different name from readAgreements().
 * The latter remains the compatibility reader for the Initiative module and
 * older public links until Initiative data is migrated from CSV.
 */
function readPublishedAgreements(string $language = 'ar'): array {
    $language = strtolower(trim($language)) === 'en' ? 'en' : 'ar';

    try {
        require_once __DIR__ . '/../../repositories/PublicAgreementRepository.php';

        $repository = new PublicAgreementRepository();
        $rows = $repository->findPublished();
    } catch (Throwable $error) {
        error_log(
            'Unable to load the public PostgreSQL Agreement catalogue: '
            . $error->getMessage()
        );

        // Fail closed. A database outage must never reveal draft or workflow
        // data, and must not silently restore the retired CSV catalogue.
        return [];
    }
    ...
    foreach ($rows as $row) {
        $reference = trim((string) ($row['public_reference'] ?? ''));
        ...
        $agreements[$reference] = [
            ...
            'agreement_name' => trim((string) (
                $language === 'ar' && !empty($row['title_ar'])
                    ? $row['title_ar']
                    : ($row['title'] ?? '')
            )),
            'agreement_type' => trim((string) ($row['agreement_type'] ?? '')),
            'partner_entity' => trim((string) ($row['partner_names'] ?? '')),
            'entity_type' => trim((string) ($row['partner_types'] ?? '')),
            'country' => trim((string) ($row['countries'] ?? '')),
            'start_date' => trim((string) ($row['start_date'] ?? '')),
            'end_date' => trim((string) ($row['end_date'] ?? '')),
            'auto_renew' => !empty($row['auto_renew']) && !in_array($row['auto_renew'], ['f', 'false', '0'], true)
                ? ($language === 'en' ? 'Yes' : 'نعم')
                : ($language === 'en' ? 'No' : 'لا'),
            'owner_entity' => trim((string) ($row['owner_unit'] ?? '')),
            'status_code' => $statusCode,
            'status' => $statusLabels[$statusCode][$language] ?? $statusCode,
            'admin_status' => $language === 'en' ? 'Approved' : 'معتمد',
            'summary' => trim((string) ($row['description'] ?? '')),
            'agreement_summary' => trim((string) ($row['description'] ?? '')),
            'goals' => trim((string) ($row['objectives'] ?? '')),
            'expected_value' => trim((string) ($row['expected_value'] ?? '')),
            'focus_area' => trim((string) ($row['focus_areas'] ?? '')),
            'sdgs' => trim((string) ($row['sdgs'] ?? '')),
            'rankings' => trim((string) ($row['rankings'] ?? '')),
            'city' => trim((string) ($row['cities'] ?? '')),
            'students_exchanged' => trim((string) ($row['students_exchanged'] ?? '')),
            'faculty_exchanged' => trim((string) ($row['faculty_exchanged'] ?? '')),
            'joint_programs' => trim((string) ($row['joint_programs'] ?? '')),
            'agreement_signing_link' => trim((string) ($row['signing_link'] ?? '')),
            'approved_at' => $approvedDate,
            'partner_website' => trim((string) ($row['partner_website'] ?? '')),
            'source' => 'postgresql',
        ];
    }
