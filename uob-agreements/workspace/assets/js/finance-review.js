(function () {
    ...
    const STEP_KEY = 'FINANCE_REVIEW';
    const elements = {
        alert: document.getElementById('finance-review-alert'),
        feedback: document.getElementById('finance-review-feedback'),
        loading: document.getElementById('finance-review-loading'),
        content: document.getElementById('finance-review-content'),
        comments: document.getElementById('finance-comments'),
        changeReason: document.getElementById('finance-change-reason'),
        approve: document.querySelector('[data-approve-finance]'),
        approveLabel: document.querySelector('[data-approve-finance-label]'),
        requestChanges: document.querySelector('[data-request-finance-changes]'),
        spinner: document.querySelector('[data-finance-spinner]'),
    ...
    function showError(error, hideContent = false) {
        ...
        elements.alert.textContent = error.message || 'The Finance review could not be completed.';
        ...
        try {
            ...
            if (!canReview(user)) {
                throw new AgreementApi.ApiError(
                    'You do not have permission to perform Finance Agreement reviews.',
                    403,
                    null
                );
            }
            ...
            if (!state.assignment) {
                throw new AgreementApi.ApiError(
                    'This Finance review is not currently assigned to you.',
                    403,
                    null
                );
            }
    ...
    elements.approve.addEventListener('click', async () => {
        ...
        if (!await WorkspaceDialog.confirm('Approve the Finance review for this Agreement?', { confirmLabel: 'Approve Finance review' })) {
            return;
        }
        ...
        try {
            ...
            complete('Finance review approved successfully. The workflow moved to its next required stage.');
        } catch (error) {
            showError(error);
            setBusy(false);
        }
    ...
    elements.requestChanges.addEventListener('click', async () => {
        ...
        if (!await WorkspaceDialog.confirm('Send this Finance change request to the VP for routing?', { confirmLabel: 'Send change request' })) {
            return;
        }
        ...
        try {
            ...
            complete('Finance changes requested. The Agreement was routed to the VP for a decision.');
        } catch (error) {
            showError(error);
            setBusy(false);
        }
